...
export function GamesToStudyPage() {
    return <NextRequireAuth Component={AuthGamesToStudyPage} />;
}

function AuthGamesToStudyPage({ user }: { user: User }) {
    const t = useTranslations('learn.gamesToStudy');
    const api = useApi();
    ...
    useEffect(() => {
        if (!listRequest.isSent()) {
            ...
            api.listGamesByOwner('games_to_study')
                .then((res) => {
