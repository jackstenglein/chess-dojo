const devUser = cy.dojo.env('dev_username');
...
describe('Round Robin Submit Results Page', () => {
    beforeEach(() => {
        ...
        cy.intercept(
            ...
            {
                statusCode: 200,
                body: { tournaments: [mockTournament] },
            }
        ).as('listRoundRobins');
    ...
    it('loads the round robin page', () => {
        ...
        cy.contains('button', 'Submit Game').should('be.visible').wait(500).click();

        cy.get('[role="dialog"]').should('be.visible').as('submitDialog');

        // From /backend/roundRobinService/submitGame.ts
        cy.intercept(
            {
                method: 'POST',
                url: '**/tournaments/round-robin/submit-game',
            },
            {
                statusCode: 400,
                body: {
                    message: 'No pairing found for this game. Make sure you play all games under the usernames in the tournament. Contact support if you are sure the game is correct.',
                },
            }
        ).as('submitGames');

        cy.get('@submitDialog').within(() => {
            cy.wait(500);
            cy.get('input[type="text"]').type(game, { delay: 50 });
            cy.contains('button', 'Submit').click();
        });

        cy.wait('@submitGames').then((interception) => {
